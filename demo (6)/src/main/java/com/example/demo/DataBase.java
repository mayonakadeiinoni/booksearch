...
public class DataBase {
	public DataBase() {//コンストラクタ、テーブル生成
		...
		con = Connect();
		try {
			String sql = "create table if not exists cache(isbn TEXT REFERENCES books,ses_id TEXT REFERENCES sessions,page INTEGER,PRIMARY KEY(isbn,ses_id,page))";
			Statement stm = con.createStatement();
			stm.executeUpdate(sql);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void Insert(BookInfo book, String ses_id, /*int max_page,*/ int page) {
		Connection con = Connect();
		...
		con=Connect();
		try {
			pstmt = con.prepareStatement("insert into cache(isbn,ses_id, page) values(?,?,?)");
			pstmt.setString(1, book.getIsbnNum());
	...
	public List<BookInfo> BookSelect(String ses_id, int page) {
		...
		try {
			...
			PreparedStatement pstmt;
			pstmt = con.prepareStatement(
					"SELECT * FROM (SELECT * FROM cache WHERE ses_id=? AND page=?) NATURAL JOIN books NATURAL JOIN (SELECT isbn,group_concat(authorName,'/') FROM authors GROUP BY isbn) NATURAL JOIN (SELECT isbn,group_concat(genreName,'/') FROM genres GROUP BY isbn)");
			pstmt.setString(1, ses_id);
			pstmt.setInt(2, page);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	*/
	public List<BookInfo> selectWithSelector(String ses_id, String selector, int limit, int offset) {
		...
		try {
			...
			PreparedStatement pstmt;
			pstmt = con.prepareStatement(
					"SELECT * FROM (SELECT * FROM cache WHERE ses_id=?) NATURAL JOIN books NATURAL JOIN (SELECT isbn,group_concat(authorName,'/') FROM authors GROUP BY isbn) NATURAL JOIN (SELECT isbn,group_concat(genreName,'/') FROM genres GROUP BY isbn) WHERE "+selector+" LIMIT ? OFFSET ?;");
			pstmt.setString(1, ses_id);
