...
public class Util {
	...
	public void setData(BooksInfos infos,long id/**TEMP*/,int page/**/) {
		...
		try {
			...
			for(BookInfo info:infos.getBookInfoList()) {
				...
				*/
				database.Insert(info, id+"", page,i++);
	...
	public List<BookInfo> getData(long id,int page,int pageSize) {
		return database.BookSelect(""+id, pageSize,pageSize*(page-1));
	}
	...
	public List<BookInfo> getSelectedData(long id,int page,int pageSize,String selector) {
		return database.selectWithSelector(""+id, selector, pageSize, pageSize*(page-1));
	}
